...
import { makeKey }                            from "../utils.js?v=1790420518";
import { renderScoreboard, updateScoreboard } from "./scoreboard.js?v=1790420518";
import { updateTurn, updateTurnUI }           from "./turnManager.js?v=1790420518";
import { endGame }                            from "./gameEnd.js?v=1790420518";
import { audioManager }                       from "../audio/audioManager.js?v=1790420518";
import { checkSquaresAround }                 from "../core/logic.js?v=1790420518";
import { onlineManager }                      from "../firebase.js?v=1790420518";
import { generateSpecialSquares, getElementAt, ELEMENTS, setElementMap, getElementMap } from "../core/specialSquares.js?v=1790420518";
import { resetPowers, addPower, getEffect, clearEffect, consumePower, setEffect, hasPower } from "../core/powers.js?v=1790420518";
import { refreshInventory } from "./powersUI.js?v=1790420518";
import { maybeShowTutorial } from "./powerTutorial.js?v=1790420518";
import { isTimerEnabled, startTurnTimer, stopTurnTimer, cutBank, getTimerMode, getBank, setBank } from "./turnTimer.js?v=1790420518";
import { resetMatchCoins, addMatchCoins } from "../core/wallet.js?v=1790420518";
...
function buildSpecialElements(cfg) {
  ...
  if (cfg.aiMode === 'online' && cfg._sharedElementMap) {
    ...
    // يبثّها أصغر رقم لاعب حاضر (وليس "1" ثابتاً — قد يكون غادر قبل البدء)
    if (cfg.aiMode === 'online') {
      let owner = 1;
      if (cfg.multiPlayers) {
        const nums = Object.values(cfg.multiPlayers)
...
export function handleEdgeClick(obj, cfg, isOpponentMove=false, forcedPlayer=null, silent=false) {
  // انتهت المباراة (طبيعياً أو بخروج الخصم أو نفاد الوقت): لا خطوط بعدها — لا نقرة ولا حركة
  // متأخرة (كانت تسمح بإكمال اللوحة بعد نهاية بالخروج فتُعاد النافذة بفائز آخر) — v35.6
  ...
  if ((state.squaresFilled || 0) >= total) {
    // إرسال الحركة الأخيرة بالقناة الصحيحة (الجماعي له سجل moves منفصل!)
    if (cfg.aiMode==='online' && !isOpponentMove) {
      const myBank = getTimerMode()==='bank' ? getBank(player) : null;
      if (cfg.multiPlayers) onlineManager.pushMultiMove(obj.key, player, Date.now(), myBank);
      else onlineManager.pushMove(obj.key, Date.now(), myBank);
    }
    state.gameFinished = true; // المباراة انتهت طبيعياً (الخروج بعدها ليس انسحاباً)
  ...
  if (cfg.aiMode==='online' && !isOpponentMove) {
    ...
    // 🕐 الساعة المركزية: نحدّث المرجع (نخصم المستهلك من بنكي، نبدأ دور التالي)
    // نحدّث فقط إذا تغيّر الدور فعلاً (السمكة/إكمال المربع = نفس اللاعب، لا نعيد ضبط turnStartAt)
    if (getTimerMode()==='bank' && nextPlayer !== player) {
      onlineManager.pushClock(player, getBank(player), nextPlayer);
    }
  }
...
export function nextActivePlayer(current, cfg) {
...
export function triggerAI(cfg) {
  if (cfg.aiMode !== 'ai') return; // الكمبيوتر يلعب فقط في وضع "ضد الكمبيوتر"
...
export function activatePower(elementType, player, cfg) {
  ...
  if (elementType === 'water') {
    ...
    // نحدّد الخصم (التالي النشط)
    const opponent = nextActivePlayer(player, cfg);
    if (getTimerMode() === 'bank') {
      // نمط البنك: قصّ فوري ودائم من بنك الخصم
      const cutVal = cutBank(opponent, 5);
      // الساعة المركزية: نكتب القيمة الجديدة في المرجع (يراها الجميع فوراً)
      if (cfg.aiMode === 'online') {
        const newVal = (typeof cutVal === 'number') ? cutVal : getBank(opponent);
        onlineManager.updateClockBank(opponent, newVal);
      }
    } else {
      // نمط الدور: علامة قصّ على دوره القادم
...
//  أونلاين
// ═══════════════════════════════════════════════════════
// إذا الدور الحالي عند لاعب منسحب: ننقله لأول لاعب نشط (حساب حتمي — متطابق عند الجميع)
export function skipInactiveTurn(cfg) {
  const players = cfg.multiPlayers;
  if (!players) return;
  const cur = state.currentPlayer;
  const isActive = Object.values(players).some(p => p.num === cur && p.active !== false);
  if (isActive) return;
